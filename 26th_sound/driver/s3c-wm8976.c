...
static void wm8976_write_reg(unsigned char reg, unsigned int data)	//reg_addr 7bit，data 9bit
{
	int i;
	unsigned long flags;
	...
	for (i = 0; i < 16; i++) {
		...
		{
			s3c2410_gpio_setpin(S3C2410_GPB4,0);	// sclk
			s3c2410_gpio_setpin(S3C2410_GPB3,0);	// sdin
			udelay(1);
			s3c2410_gpio_setpin(S3C2410_GPB4,1);			
		}
		val = val << 1;
	...
	local_irq_restore(flags);		// 做完之后开中断
}
				...
				unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SOUND_MIXER_WRITE_VOLUME:
			...
			/* ioctl: val越大表示音量越大, 0-100
			 * UDA1341: 寄存器的值越小音量越大
			 * WM8976: 52,53号寄存器bit[5:0]表示音量, 值越大音量越大, 0-63(111111)
			 */
			uda1341_volume = (((val & 0xff) + 1) * 63) / 100;	// x = (val x 63) / 100, +1? 可能0-99
			wm8976_write_reg(52, (1<<8)|uda1341_volume);	// 左通道
			wm8976_write_reg(53, (1<<8)|uda1341_volume);	// 右通道
			//uda1341_l3_address(UDA1341_REG_DATA0);			// 先设置成UDA1341_REG_DATA0状态，然后发出音量数据
...
{
	// see wm8976.pdf POWER SUPPLIES, p80
	uda1341_volume = 57;	// 根据wm8976.pdf设置默认值
	uda1341_boost = 0;

	// reg_addr: see wm8976.pdf REGISTER MAP
	wm8976_write_reg(0, 0);		// reg_addr=0: software reset, data=0: p78 write any value	// 上电之后肯定是要复位的，里面有复位电路

	// reg_addr顺序R3->R1->R2? see wm8976.pdf POWER SUPPLIES & jz2440 pcb.
	wm8976_write_reg(0x3, 0x6f);// bit3 RMIXEN, wm8976.pdf "Stereo CODEC with Speaker Driver", 如图: "LEFT/RIGHT OUTPUT MIXER"
															// OUT2的左/右声道打开
															// 左/右通道输出混音打开
															// 左/右DAC打开
	wm8976_write_reg(0x1, 0x1f);// biasen,BUFIOEN.VMIDSEL=11b
															// bit4 mic偏置使能 => mic的驱动电源来自wm8976的引脚MICBIAS, 平时为了省电, MICBIAS引脚是不输出的
