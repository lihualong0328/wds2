...
static volatile struct s3c_dma_regs *dma_regs;	//volatile防止被优化。
...
static int s3c_dma_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case MEM_CPY_DMA :
		{
			...
			dma_regs->dcon       = (1<<30)|(1<<29)|(0<<28)|(1<<27)|(0<<23)|(0<<20)|(BUF_SIZE<<0);  /* src/dest都是AHB,使能中断,单个传输,一次性传输完,软件触发, */
			...
			/* 如何知道DMA什么时候完成? */
			/* 休眠 */
			wait_event_interruptible(dma_waitq, ev_dma);	//如果ev_dma == 0,休眠。

			if (memcmp(src, dst, BUF_SIZE) == 0)	//这和nodma一样，同样占用cpu。
			{
