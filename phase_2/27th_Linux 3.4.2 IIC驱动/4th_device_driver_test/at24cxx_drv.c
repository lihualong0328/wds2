...
static ssize_t at24cxx_read(struct file * file, char __user *buf, size_t count, loff_t *off)
{
	...
	// 之前用i2c_transfer时，构造了两个struct i2c_msg, 一个写地址, 一个读数据
	// SMBus Read Byte:  i2c_smbus_read_byte_data()
	// S Addr Wr [A] Comm [A] S Addr Rd [A] [Data] NA P ==> 满足at24c08.pdf"Byte Write" Figure 5: Random Read
...
static ssize_t at24cxx_write(struct file *file, const char __user *buf, size_t count, loff_t *off)
{
	...
	// 设备驱动调用核心层提供的函数操作, 如图: 
	// 之前用过i2c_transfer, 这里用smbus => 
	// 看smbus-protocol文档和at24c08.pdf"Byte Write"
	// SMBus Send Byte:  i2c_smbus_write_byte()
	// S Addr Wr [A] Data [A] P => 不行, at24c08.pdf"Byte Write"中还要发
	// SMBus Write Byte:  i2c_smbus_write_byte_data()
	// S Addr Wr [A] Comm [A] Data [A] P => 满足at24c08.pdf"Byte Write"
