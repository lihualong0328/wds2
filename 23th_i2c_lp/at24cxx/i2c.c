...
void i2c_write(unsigned int slvAddr, unsigned char *buf, int len)
{
    ...
    IICDS   = slvAddr;  // 
    IICSTAT = 0xf0; //设置 IICSTAT 即发送start信号启动并把 IICDS设备地址发出去, 之后产生中断 I2CIntHandle(void)
    
    /* 等待直至数据传输完毕 */    
    while (g_tS3C24xx_I2C.DataCount != -1);//可在此休眠，在中断服务程序I2CIntHandle(void)中当if((g_tS3C24xx_I2C.DataCount--) == 0)唤醒即可
}
...
void i2c_read(unsigned int slvAddr, unsigned char *buf, int len)
{
    ...
    IICDS        = slvAddr;	// 发出设备地址后产生中断；I2CIntHandle();
    IICSTAT      = 0xb0;    // 主机接收，发出start信号启动
 ...
 * I2C中断服务程序
 * 根据剩余的数据长度选择继续传输或者结束
 * kernel中，I2CIntHandle(void)等已做好，只需写设备层
 */
void I2CIntHandle(void)
{
    ...
    {    
        case WRDATA:
        {
            ...
            IICDS = g_tS3C24xx_I2C.pData[g_tS3C24xx_I2C.Pt++];	// 取出第一个数据
            
            // 将数据写入IICDS后，需要一段时间才能出现在SDA线上
            for (i = 0; i < 10; i++);   

            IICCON = 0xaf;  // 恢复I2C传输，再次发送，完后继续产生中断
            break;
        }

        case RDDATA:
        {
            if (g_tS3C24xx_I2C.Pt == -1)    // 表示中断是发出设备地址后产生的
            {
                // 这次中断是发送I2C设备地址后发生的，没有数据
                // 以下是启动收数据
                g_tS3C24xx_I2C.Pt = 0;
                if(g_tS3C24xx_I2C.DataCount == 1)
                   IICCON = 0x2f;   // 恢复I2C传输，开始接收数据，接收到数据后不发出ACK，即只读1个长度；
                else 
                   IICCON = 0xaf;   // 恢复I2C传输，开始接收数据
                break;//处理完发设备地址产生的中断后，break，等待 IICCON设置的接收数据产生的中断
            }

            g_tS3C24xx_I2C.pData[g_tS3C24xx_I2C.Pt++] = IICDS;//读出接收到的数据
            g_tS3C24xx_I2C.DataCount--;
            
            if (g_tS3C24xx_I2C.DataCount == 0)//i2c_read 里也退出 while
            {
                // 下面两行恢复I2C操作，发出P信号
                IICSTAT = 0x90;
                IICCON  = 0xaf;
